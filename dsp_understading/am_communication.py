...
def encode_and_modulate(
    ...
    h = (
        np.sinc(t / symbol_length)
        * np.cos(np.pi * alpha * t / symbol_length)
        / (1 - (2 * alpha * t / symbol_length) ** 2)
    )
...
def demodulate_and_decode(
    modulated, sample_rate=SAMPLE_RATE, carrier_freq=CARRIER_FREQ, bit_rate=BIT_RATE
):
