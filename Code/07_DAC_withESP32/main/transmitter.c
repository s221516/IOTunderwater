...
void create_carrier_wave(const float* time_array, int* carrier_wave, int total_samples)
{
    for(int i = 0; i < total_samples; i++) {
        ...
        //following is chat code
        carrier_wave[i] = (int)fmin(fmax(round(128 * sin(2 * M_PI * CARRIER_FREQ * time_array[i])) + 128, 0), 255);
...
{   
    ...
    for(int i = 0; i < total_samples; i++) {
        ...
        // vTaskDelay(pdMS_TO_TICKS(1000/SAMPLE_RATE)); THIS DELAY IS NOT WORKING SLOWEST IS 1 MS 
        ets_delay_us(1000000 / SAMPLE_RATE);

    
        // Company gang here. ets_delay_us works by actively polling a 
        //timer until that timer value is larger than a given value. 
        //This means it should always wait the given amount of uS. 
        //Normally, it will wait for exactly that amount of time before it returns, 
        //but as Peter already indicated, you're running on a multitasking system 
        //so you may have your task switched or an ISR kicking in, causing the call to return 
        //a fair bit later. Statistically, this should be more of an 
        //exception than a rule though, given you only wait for a few uS.
        //https://www.esp32.com/viewtopic.php?t=16100
